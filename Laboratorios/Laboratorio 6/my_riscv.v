module my_riscv (
  ...
  register_file rv_rf (
    ...
    .wr_en(wr_en & state_execute2),
    .wr_index(rd),
    //.wr_data(is_load ? ddata_in : alu_out), 
     .wr_data(is_load ? ddata_in : ((is_jal | is_jalr) ? pc_plus_4 : alu_out)), // MUX
    .rd_index1(rs1),
    .rd_data1(src1_value),
    .rd_index2(rs2),
    .rd_data2(src2_value)
    ...
    assign imm = is_i_instr ? { {21{IR[31]}}, IR[30:20] } :
                 is_s_instr ? { {20{IR[31]}}, IR[31:25], IR[11:7] } :
    ...
    //wire imm_valid = is_i_instr | is_s_instr | is_b_instr | is_u_instr | is_j_instr | is_jalr;
    ...
    // Registro que almacena PC+4 en los ciclos fetch/decode
    ...
    assign next_pc = is_jalr ? alu_out : ((is_j_instr || taken_br) ? br_tgt_pc : pc_plus_4);
    // --- MUX para la entrada B de la ALU ---
    assign alu_a = is_auipc ? pc : is_lui ? 32'b0: src1_value;
    // Aquí ya se implemento la figura 14 del documento de la guía
    assign alu_b = imm_valid ? imm : src2_value; //

    // NUEVO: ASIGNACIONES PARA LA MEMORIA RAM
    // ==========================================
    assign daddr = alu_out;
    assign ddata_out = src2_value;
    ...
    alu_riscv rv_alu (
        ...
        .ctrl({
            is_add | is_addi | is_lui | is_auipc | is_load | is_s_instr | is_jalr,
            is_sub, 
            is_sll | is_slli, 
            is_slt | is_slti, 
            is_sltu| is_sltui, 
    ...
    assign is_jal = is_j_instr;
    assign is_jalr = (opcode == 7'b1100111) && (funct3 == 3'b000);

    // Cargas en formato I (LB, LH, LW, LBU, LHU) sobre opcode 0000011
    ...
    assign wr_en = (is_r_instr | is_i_instr | is_u_instr | is_j_instr | is_load) & is_valid;
